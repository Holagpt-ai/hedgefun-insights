...
-- Copies the last same-session candidate payload before park_closed empties
-- radar_v22_candidates. The live table stays empty after close.
...
ALTER TABLE public.radar_v22_closed_snapshot ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Closed-session Radar snapshot is publicly readable"
  ON public.radar_v22_closed_snapshot;
CREATE POLICY "Closed-session Radar snapshot is publicly readable"
  ON public.radar_v22_closed_snapshot FOR SELECT USING (true);

REVOKE ALL ON TABLE public.radar_v22_closed_snapshot FROM PUBLIC;
GRANT SELECT ON TABLE public.radar_v22_closed_snapshot TO anon, authenticated;
GRANT ALL ON TABLE public.radar_v22_closed_snapshot TO service_role;
...
BEGIN
  ...
  -- Last same-session display copy. The live candidate delete below still runs.
  -- A later empty closed write must not erase a snapshot already taken today.
