-- Scanner Intelligence Stack V1 (#1 Radar Event Engine + #2 Intraday Participation):
-- extends replace_radar_v22_candidates_v1 to persist new candidate columns and event types.
-- Apply after 20260926130000_intraday_participation_v1.sql.
...
CREATE OR REPLACE FUNCTION public.replace_radar_v22_candidates_v1(
  ...
  p_market_data_provider text DEFAULT NULL,
  p_market_data_feed_mode text DEFAULT NULL,
  p_feed_latency_ms integer DEFAULT NULL,
  p_feed_connection_state text DEFAULT NULL,
  p_feed_last_message_at timestamptz DEFAULT NULL,
  p_feed_telemetry_stale boolean DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  DELETE FROM public.radar_v22_closed_snapshot
  WHERE trading_date IS DISTINCT FROM p_trading_date;

  IF p_session_kind = 'closed' AND v_len = 0
     AND EXISTS (
       SELECT 1 FROM public.radar_v22_candidates
       WHERE trading_date = p_trading_date
     ) THEN
    DELETE FROM public.radar_v22_closed_snapshot;
    INSERT INTO public.radar_v22_closed_snapshot
    SELECT c.*, 'closed_session', p_synced_at
    FROM public.radar_v22_candidates AS c
    WHERE c.trading_date = p_trading_date;
  END IF;
  ...
  IF v_len > 0 THEN
    INSERT INTO public.radar_v22_candidates (
      ...
      acceleration_5m, rvol_5m, volume_velocity, volume_acceleration_pct,
      session_high, session_low, distance_from_hod_pct,
      ...
      primary_scanner_event, primary_scanner_event_at, scanner_events,
      promotion_reason, radar_event_lifecycle, radar_engine_events,
      time_adjusted_rvol, volume_5m, volume_15m, volume_60m,
      volume_velocity_5m, volume_velocity_15m, volume_velocity_60m,
      dollar_volume_velocity_5m, participation_state,
      participation_baseline_session_count, participation_calculated_at,
      participation_source_as_of,
      previous_close, prior_session_volume,
      updated_at
    )
    SELECT
      upper(trim(e ->> 'symbol')),
      ...
      NULLIF(e ->> 'rvol_5m', '')::numeric,
      NULLIF(e ->> 'volume_velocity', '')::numeric,
      NULLIF(e ->> 'volume_acceleration_pct', '')::numeric,
      ...
      NULLIF(e ->> 'primary_scanner_event', ''),
      NULLIF(e ->> 'primary_scanner_event_at', '')::timestamptz,
      CASE WHEN e ? 'scanner_events' THEN e -> 'scanner_events' ELSE NULL END,
      CASE WHEN e ? 'promotion_reason' THEN e -> 'promotion_reason' ELSE NULL END,
      NULLIF(e ->> 'radar_event_lifecycle', ''),
      CASE WHEN e ? 'radar_engine_events' THEN e -> 'radar_engine_events' ELSE NULL END,
      NULLIF(e ->> 'time_adjusted_rvol', '')::numeric,
      NULLIF(e ->> 'volume_5m', '')::numeric,
      NULLIF(e ->> 'volume_15m', '')::numeric,
      NULLIF(e ->> 'volume_60m', '')::numeric,
      NULLIF(e ->> 'volume_velocity_5m', '')::numeric,
      NULLIF(e ->> 'volume_velocity_15m', '')::numeric,
      NULLIF(e ->> 'volume_velocity_60m', '')::numeric,
      NULLIF(e ->> 'dollar_volume_velocity_5m', '')::numeric,
      NULLIF(e ->> 'participation_state', ''),
      NULLIF(e ->> 'participation_baseline_session_count', '')::integer,
      NULLIF(e ->> 'participation_calculated_at', '')::timestamptz,
      NULLIF(e ->> 'participation_source_as_of', '')::timestamptz,
      CASE
        WHEN NULLIF(e ->> 'previous_close', '') IS NULL THEN NULL
        WHEN (e ->> 'previous_close')::numeric > 0 THEN (e ->> 'previous_close')::numeric
        ELSE NULL
      END,
      CASE
        WHEN NULLIF(e ->> 'prior_session_volume', '') IS NULL THEN NULL
        WHEN (e ->> 'prior_session_volume')::numeric > 0 THEN (e ->> 'prior_session_volume')::numeric
        ELSE NULL
      END,
      p_synced_at
  ...
  INSERT INTO public.radar_v22_feed_state (
    ...
    market_data_provider, market_data_feed_mode, feed_latency_ms,
    feed_connection_state, feed_last_message_at, feed_telemetry_stale
  ) VALUES (
    'current',
    NULL,
    'empty',
    ...
    p_last_receive_at,
    p_synced_at,
    p_market_data_provider,
    p_market_data_feed_mode,
    p_feed_latency_ms,
    p_feed_connection_state,
    p_feed_last_message_at,
    COALESCE(p_feed_telemetry_stale, false)
  )
  ON CONFLICT (state_key) DO UPDATE SET
    ...
    market_data_provider = COALESCE(EXCLUDED.market_data_provider, public.radar_v22_feed_state.market_data_provider),
    market_data_feed_mode = COALESCE(EXCLUDED.market_data_feed_mode, public.radar_v22_feed_state.market_data_feed_mode),
    feed_latency_ms = COALESCE(EXCLUDED.feed_latency_ms, public.radar_v22_feed_state.feed_latency_ms),
    feed_connection_state = COALESCE(EXCLUDED.feed_connection_state, public.radar_v22_feed_state.feed_connection_state),
    feed_last_message_at = COALESCE(EXCLUDED.feed_last_message_at, public.radar_v22_feed_state.feed_last_message_at),
    feed_telemetry_stale = COALESCE(EXCLUDED.feed_telemetry_stale, public.radar_v22_feed_state.feed_telemetry_stale),
