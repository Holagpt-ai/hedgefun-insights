...
Deno.test("loadEnv does not require SUPABASE_SERVICE_ROLE_KEY or SUPABASE_URL", () => {
  ...
  assertEquals(env.marketDataFeedMode, "auto");
  assertEquals(env.massiveWsMode, "realtime");
...
Deno.test("loadEnv RADAR_SENTINEL_ENABLED defaults true and accepts true/false", () => {
  const defaults = loadEnv((k) => {
    if (k === "POLYGON_API_KEY") return "poly";
    if (k === "RADAR_BRIDGE_URL") return "https://example.supabase.co/functions/v1/radar-worker-bridge";
    if (k === "RADAR_WORKER_SECRET") return "secret";
    return undefined;
  });
  assertEquals(defaults.radarSentinelEnabled, true);
  const on = loadEnv((k) => {
    if (k === "POLYGON_API_KEY") return "poly";
    if (k === "RADAR_BRIDGE_URL") return "https://example.supabase.co/functions/v1/radar-worker-bridge";
    if (k === "RADAR_WORKER_SECRET") return "secret";
...
Deno.test("loadEnv RADAR_PERSISTENCE_V2_ENABLED defaults true", () => {
  const env = loadEnv((k) => {
    if (k === "POLYGON_API_KEY") return "poly";
    if (k === "RADAR_BRIDGE_URL") return "https://example.supabase.co/functions/v1/radar-worker-bridge";
    if (k === "RADAR_WORKER_SECRET") return "secret";
