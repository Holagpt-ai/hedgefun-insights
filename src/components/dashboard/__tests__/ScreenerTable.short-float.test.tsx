...
describe("ScreenerTable Short Float presentation", () => {
  it("keeps Discovery order and does not add a Short Float column", () => {
    const tab = getScreenerTabById("day_trade_radar");
    expect(tab?.columns.some((column) => column.key === "short_float")).toBe(false);
    render(
      <MemoryRouter>
        <ScreenerTable
          tab={tab!}
    ...
    expect(screen.queryByRole("columnheader", { name: /Short Float/ })).not.toBeInTheDocument();
  ...
  it("renders unavailable short float on mobile without hiding Trigger Time", () => {
    const tab = getScreenerTabById("day_trade_radar");
    render(
      <MemoryRouter>
        <ScreenerTable
