      ...
      {(visible) => (
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          {visible.map((g) => {
            const c = g.primary;
            return (
              <div key={g.key} className="flex flex-col gap-2 rounded-xl border bg-card p-3">
                <div className="flex flex-wrap items-center gap-2">
                  {!g.grouped && <span className="text-sm font-semibold">{c.symbol}</span>}
                ...
                {g.grouped && (
                  <p className="text-xs text-muted-foreground">
                    ...
                    {g.symbols.map((sym, i) => {
                      const routes = symbolRoutes(sym);
                      const chip = (
                        <span key={sym} className="font-semibold text-foreground">
                          {sym}
                        </span>
                      );
                      return (
                        <span key={sym}>
                          {i > 0 ? " · " : ""}
                          {routes ? (
                            <Link to={routes.catalyst} className="font-semibold text-foreground hover:underline">
                              {sym}
                            </Link>
                          ) : (
                            chip
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {!g.grouped && <PreMarketSymbolActions symbol={c.symbol} />}
                  {c.source_url && (
                    <a
                      href={c.source_url}
                      target="_blank"
