...
describe("screener display-field enrichment", () => {
  it("fills Move from verified screener_results when Sentinel leaves change_percent null", () => {
    ...
    const lookup = buildDisplayFieldLookup([donor("HAIN", "volume_spikes")], null);
  ...
  it("does not overwrite existing Sentinel display fields", () => {
    ...
    const lookup = buildDisplayFieldLookup([donor("AAA", "day_trade_radar", { change_percent: 20 })], null);
  ...
  it("never backfills legacy rvol", () => {
    ...
    const donorRow = donor("CCC", "day_trade_radar", { rvol: 4.2 } as Partial<ScreenerResultRow>);
  ...
  it("prefers donors from the active tab when scores tie", () => {
    ...
    const lookup = buildDisplayFieldLookup(
      [
        donor("DDD", "gainers_losers", { change_percent: 11 }),
        donor("DDD", "volume_spikes", { change_percent: 22 }),
      ],
      null,
      "volume_spikes",
    );
  ...
  it("gap_percent copies only when allowGap is true", () => {
    ...
    const lookup = buildDisplayFieldLookup(
      [donor("FGP", "gappers", { gap_percent: 7.2 })],
      null,
    );
