...
const DAILY_FIELDS = [
  "securityId", "sessionDate", "observedSymbol", "exchange", "open", "high", "low", "close",
  "volume", "dollarVolume", "previousClose", "movePct", "source", "quality", "provenance",
] as const;
...
function bridgeRpcError(error: Error): never {
...
export class BridgeSecurityIntelligenceRepository {
  ...
  async getDailyHistory(securityId: string, sessionDate: string): Promise<SecurityDailyHistory | null> {
    ...
    const res = await this.bridge.call("historical_get_daily_history", {
      security_id: securityId,
      session_date: sessionDate,
    });
    return res.row ? mapDaily(res.row as Record<string, unknown>) : null;
  }
  ...
  async listEpisodes(securityId?: string): Promise<readonly MarketBehaviorEpisode[]> {
    ...
    const rows = await this.bridge.fetchAllRows("historical_list_episodes", { security_id: securityId });
  ...
  async getJob(jobId: string): Promise<SecurityBackfillJob | null> {
    const res = await this.bridge.call("historical_get_job", { job_id: jobId });
    return res.job ? mapJob(res.job as Record<string, unknown>) : null;
  }
  ...
  private async flushDaily(): Promise<void> {
    ...
    for (const group of chunks(queued, this.batchSize)) {
      try {
        await this.bridge.call("historical_apply_daily_batch", { p_rows: group.map(dailyToRpcRow) });
      } catch (error) {
        bridgeRpcError(error instanceof Error ? error : new Error(String(error)));
      }
    }
  }
  ...
  private async flushEpisodes(): Promise<void> {
    ...
    for (const group of chunks(queued, this.batchSize)) {
      await this.bridge.call("historical_apply_episode_batch", { p_rows: group.map(episodeToRpcRow) });
    }
  }

  private async insertJob(row: SecurityBackfillJob): Promise<void> {
    await this.bridge.call("historical_create_job", {
      job_id: row.jobId,
  ...
  private async updateJob(row: SecurityBackfillJob): Promise<void> {
    await this.bridge.call("historical_update_job", {
      job_id: row.jobId,
      state: row.state,
      cursor_date: row.cursorDate,
      cursor_token: row.cursorToken,
