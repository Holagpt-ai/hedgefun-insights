      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div style={card}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {topTickers.map((t) => (
                <tr key={t.ticker} style={{ borderBottom: "1px solid #e2e8f0" }}>
                  <td className="py-2 font-semibold" style={{ color: "#2563eb" }}>{t.ticker}</td>
                  <td className="py-2" style={{ color: "#64748b" }}>{t.views.toLocaleString()}</td>
                  <td className="py-2" style={{ color: "#64748b" }}>{t.unique.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {["What is a P/E ratio?", "Best stocks to buy now?", "How to read earnings?"].map((q) => (
                <p key={q} className="text-xs py-1" style={{ color: "#64748b" }}>• {q}</p>
              ))}
            </div>
          </div>
        </div>
