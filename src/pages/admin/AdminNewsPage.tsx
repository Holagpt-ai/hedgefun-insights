...
const card: React.CSSProperties = { background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 8 };
          ...
          <tbody>
            {news.map((n) => (
              <tr key={n.id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                ...
                <td className="px-4 py-3" style={{ color: "#64748b" }}>{n.source}</td>
                <td className="px-4 py-3">
                  <span className="text-xs px-2 py-0.5 rounded" style={{ background: "rgba(37,99,235,0.1)", color: "#2563eb" }}>{n.category}</span>
                </td>
                <td className="px-4 py-3" style={{ color: "#64748b" }}>{n.published_at ? new Date(n.published_at).toLocaleDateString() : "—"}</td>
                <td className="px-4 py-3">
                  <button className="text-xs mr-2" style={{ color: "#2563eb" }}>Edit</button>
                  <button className="text-xs" style={{ color: "#dc2626" }} onClick={() => handleDelete(n.id)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
