...
function mapApiToRows(tickers: any[]): AfterHoursRow[] {
  return tickers.map((t: any, i: number) => ({
    rank: i + 1,
    symbol: t.ticker || t.symbol || "",
    name: t.name || t.ticker || "",
    changePercent: t.todaysChangePerc ?? 0,
    afterhrPrice: t?.lastTrade?.p || t?.lastQuote?.P || t?.day?.c || t?.prevDay?.c || 0,
...
function AfterHoursTable({
  ...
  const columns = useMemo<ColumnDef<AfterHoursRow>[]>(
    () => [
      { accessorKey: "rank", header: "No.", size: 40, enableSorting: false, cell: ({ row }) => <span className="text-[0.875rem] tabular-nums" style={{ color: "hsl(var(--text-secondary))" }}>{row.index + 1}</span> },
      { accessorKey: "symbol", header: "Symbol", size: 70, cell: ({ getValue }) => { const s = getValue<string>(); return <button onClick={() => { trackEvent("stock_search", { ticker: s }); navigate(`/stocks/${s.toLowerCase()}`); }} className="ticker-symbol text-[0.875rem] hover:underline" style={{ color: "hsl(var(--accent-blue))" }}>{s}</button>; } },
      ...
      <div className="flex items-center gap-0 border-b border-border mb-0">
        {VIEW_TABS.map((t, i) => (
          <button key={t} className="px-3 py-2 text-[0.875rem] transition-colors relative" style={{ fontWeight: i === 0 ? 600 : 400, color: i === 0 ? "hsl(var(--accent-blue))" : "hsl(var(--text-secondary))" }}>
            {t}{i === 0 && <span className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: "hsl(var(--accent-blue))" }} />}
          </button>
        ))}
  ...
  return (
    <div className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <h1 className="text-[1.75rem] font-bold mb-4" style={{ color: "hsl(var(--text-primary))" }}>Market Movers</h1>
        <div className="flex items-center gap-0 mb-6">
          {SUB_TABS.map((t, i) => (
            <button key={t} className="px-3 py-2 text-[0.875rem] transition-colors relative" style={{ fontWeight: i === 0 ? 700 : 400, color: i === 0 ? "hsl(var(--text-primary))" : "hsl(var(--accent-blue))" }}>
              {t}{i === 0 && <span className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: "hsl(var(--text-primary))" }} />}
            </button>
          ))}
        </div>
        <IndexSparklines />
