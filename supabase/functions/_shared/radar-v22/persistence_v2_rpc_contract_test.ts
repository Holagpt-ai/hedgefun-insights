...
const LATEST_RPC_MIGRATION_NAME =
  "20260926130200_scanner_intelligence_stack_replace_rpc_v1.sql";
...
Deno.test("static: candidate INSERT persists Phase 1 momentum fields", async () => {
  const body = functionBody(await loadRpc());
...
Deno.test("static: candidate INSERT persists scanner intelligence stack fields", async () => {
  const body = functionBody(await loadRpc());
  assert(body.includes("promotion_reason, radar_event_lifecycle, radar_engine_events"));
  assert(body.includes("time_adjusted_rvol, volume_5m, volume_15m, volume_60m"));
  assert(body.includes("participation_state"));
  assert(body.includes("NULLIF(e ->> 'time_adjusted_rvol', '')::numeric"));
  assert(body.includes("'MOMENTUM_TRIGGER', 'RE_ACCELERATION'"));
});
