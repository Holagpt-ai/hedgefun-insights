...
Deno.test("metrics: explicit zero prior-day aggregate requires full OHLCV zeros", () => {
  assertEquals(
    isExplicitZeroPriorDayAggregate({
      ticker: "A",
  ...
  assertEquals(
    hasValidCurrentDayOpen({ ticker: "A", day: { o: 0 } }),
    false,
  );
  assertEquals(
    hasValidCurrentDayOpen({ ticker: "A", day: { o: -1 } }),
    false,
  );
  assertEquals(
