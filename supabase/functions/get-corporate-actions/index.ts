...
serve(async (req) => {
  ...
  try {
    ...
    const { data: claims, error: claimsError } = await (supabase.auth as unknown as { getClaims: (t: string) => Promise<{ data: { claims?: unknown } | null; error: unknown }> }).getClaims(token);
