...
BEGIN
  ...
  -- ── Genesis path: no generation/state yet; expected revision must be 0 ──
  ...
  -- Idempotent re-apply: state already at next revision with equivalent rows.
  IF v_state.revision = p_next_revision THEN
    ...
    IF NOT FOUND
       OR v_existing_eval.evaluation_status IS DISTINCT FROM p_evaluation_status
       OR v_existing_eval.candidate_stage IS DISTINCT FROM p_candidate_stage
       OR v_existing_eval.input_fingerprint IS DISTINCT FROM p_input_fingerprint
       OR v_existing_eval.p1_status IS DISTINCT FROM p_p1_status
       OR v_existing_eval.reason_codes IS DISTINCT FROM p_reason_codes
       OR v_existing_eval.metrics IS DISTINCT FROM p_metrics THEN
      RETURN jsonb_build_object(
        'ok', false,
        'outcome', 'stale_revision',
    ...
    IF v_has_transition THEN
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'market_stages_forward_partial_write';
      END IF;

      IF v_existing_tr.from_stage IS DISTINCT FROM v_tr_from
         OR v_existing_tr.to_stage IS DISTINCT FROM v_tr_to
         OR v_existing_tr.pending_start_week_end IS DISTINCT FROM v_tr_pending_start
         OR v_existing_tr.effective_week_end IS DISTINCT FROM v_tr_effective
         OR v_existing_tr.confirmed_week_end IS DISTINCT FROM v_tr_confirmed
         OR v_existing_tr.confirmation_input_fingerprint IS DISTINCT FROM v_tr_fingerprint
         OR v_existing_tr.reason_codes IS DISTINCT FROM v_tr_reason_codes
         OR v_existing_tr.metrics_snapshot IS DISTINCT FROM v_tr_metrics
         OR v_existing_tr.status IS DISTINCT FROM 'active'
         OR v_existing_tr.alert_eligible IS DISTINCT FROM false THEN
  ...
  -- Immutable evaluation insert (never UPDATE).
  SELECT e.*
    INTO v_existing_eval
    FROM public.market_stage_weekly_evaluations e
   WHERE e.generation_id = v_gen_id
  ...
  IF v_has_transition THEN
    ...
    IF FOUND THEN
      IF v_existing_tr.from_stage = v_tr_from
         AND v_existing_tr.to_stage = v_tr_to
         AND v_existing_tr.pending_start_week_end = v_tr_pending_start
         AND v_existing_tr.effective_week_end = v_tr_effective
         AND v_existing_tr.confirmed_week_end = v_tr_confirmed
         AND v_existing_tr.confirmation_input_fingerprint = v_tr_fingerprint
         AND v_existing_tr.reason_codes = v_tr_reason_codes
         AND v_existing_tr.metrics_snapshot = v_tr_metrics
         AND v_existing_tr.status = 'active'
         AND v_existing_tr.alert_eligible = false THEN
        v_outcome := 'duplicate_transition';
      ELSE
        RAISE EXCEPTION 'market_stages_forward_transition_conflict';
      END IF;
    ELSE
