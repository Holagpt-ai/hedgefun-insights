...
-- integrity-md5: 5f410ae088b768f9f84cc54490e13d40
DO $journal_rls$
DECLARE
  r record;
  t text;
  ...
  parent_fk text;
  v_digest text;
  v_expected text := '5f410ae088b768f9f84cc54490e13d40';
...
EXISTS (
    ...
    ELSIF parent_table IS NOT NULL THEN
      pred := format('EXISTS (SELECT 1 FROM public.%I p WHERE p.id = %I.%I AND p.user_id = auth.uid())', parent_table, t, parent_fk);
    ELSE
      pred := 'user_id = auth.uid()';
    END IF;
