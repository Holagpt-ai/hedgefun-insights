...
-- Reuses the existing 52-week baseline generation lifecycle and atomic publish boundary.
-- Forward-only. Additive. Does not alter HL calculations, NHL policy, or ranking.

-- ── Staging: last ~20 valid sessions per symbol per generation ──
...
-- ── Published baselines (flip with 52W generation pointer) ──
...
-- ── Idempotency for daily volume apply ──
...
-- ── screener_results explicit RVOL 20D columns ──
...
-- ── Trim helper: keep latest 20 sessions per symbol ──
...
-- ── Copy-forward: seed staged generation from current production rolling history ──
...
-- ── Daily grouped-volume apply (separate from HL RPC) ──
...
-- ── Publish volume baselines from staged history (exactly 20 sessions) ──
...
-- ── Worker bulk load from in-memory cache (zero extra provider calls) ──
...
-- ── Extend start job: clear prior generation volume staging ──
...
-- ── Extend cron finalizer: publish volume baselines atomically with HL flip ──
...
-- Extend direct replace publisher: drop stale volume generations with HL flip.
...
-- ── Extend worker staged publish start: clear prior generation volume staging ──
...
-- ── Extend worker staged publish finalizer: volume baselines flip with HL pointer ──
