-- RVOL 20D screener_results publication columns.
-- Extends replace_screener_results_generation_v1 from tab_evaluation_evidence_v1.
-- Forward-only. Legacy avg_volume/rvol remain null-forced.

DROP FUNCTION IF EXISTS public.replace_screener_results_generation_v1(jsonb, uuid, timestamptz);
DROP FUNCTION IF EXISTS public.replace_screener_results_generation_v1(jsonb, uuid, timestamptz, text);
CREATE OR REPLACE FUNCTION public.replace_screener_results_generation_v1(
  ...
  p_tab_evaluation_evidence jsonb DEFAULT NULL
)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_tab_evaluation_evidence IS NOT NULL
     AND jsonb_typeof(p_tab_evaluation_evidence) <> 'object' THEN
    RAISE EXCEPTION 'tab_evaluation_evidence must be an object';
  END IF;

  IF p_tab_evaluation_evidence IS NOT NULL THEN
    FOR v_evidence_key IN SELECT jsonb_object_keys(p_tab_evaluation_evidence)
    LOOP
      IF NOT (v_evidence_key = ANY (v_managed_tabs)) THEN
        RAISE EXCEPTION 'invalid tab_evaluation_evidence key';
      END IF;
    END LOOP;
  END IF;
  ...
  LOOP
    ...
    BEGIN
      v_avg20 := NULLIF(v_elem->>'avg_volume_20d', '')::numeric;
    EXCEPTION WHEN others THEN
      RAISE EXCEPTION 'invalid avg_volume_20d';
    END;
    BEGIN
      v_rvol20 := NULLIF(v_elem->>'rvol_20d', '')::numeric;
    EXCEPTION WHEN others THEN
      RAISE EXCEPTION 'invalid rvol_20d';
    END;
    ...
    IF v_tab = ANY (v_rvol_tabs) THEN
      IF (v_avg20 IS NULL) <> (v_rvol20 IS NULL) THEN
        RAISE EXCEPTION 'avg_volume_20d and rvol_20d must both be null or both set';
      END IF;
      IF v_avg20 IS NOT NULL AND (v_avg20 <= 0 OR v_rvol20 <= 0) THEN
        RAISE EXCEPTION 'invalid rvol_20d pair';
      END IF;
    ELSIF v_avg20 IS NOT NULL OR v_rvol20 IS NOT NULL THEN
      RAISE EXCEPTION 'rvol_20d fields reserved for supported tabs';
    END IF;
  ...
  IF v_len > 0 THEN
    ...
    SELECT
      ...
      NULLIF(e ->> 'avg_volume_20d', '')::numeric,
      NULLIF(e ->> 'rvol_20d', '')::numeric,
  ...
  ON CONFLICT (state_key) DO UPDATE SET
    ...
    tab_evaluation_evidence = EXCLUDED.tab_evaluation_evidence,
...
REVOKE ALL ON FUNCTION public.replace_screener_results_generation_v1(jsonb, uuid, timestamptz, text, jsonb)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.replace_screener_results_generation_v1(jsonb, uuid, timestamptz, text, jsonb)
  FROM anon;
REVOKE ALL ON FUNCTION public.replace_screener_results_generation_v1(jsonb, uuid, timestamptz, text, jsonb)
  FROM authenticated;
GRANT EXECUTE ON FUNCTION public.replace_screener_results_generation_v1(jsonb, uuid, timestamptz, text, jsonb)
  TO service_role;
