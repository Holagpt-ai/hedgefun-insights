-- RVOL 20D generation carry-forward P0 fix.
-- Proven failure: finalize published volume baselines for a generation that
-- owned no volume history (0 rows), then cleanup deleted the rolling history
-- accumulated under the catch-up generation. Net effect: 0% RVOL coverage and
-- no self-healing.
--
-- Fix (minimal, additive):
--   1. copy_screener_daily_volume_history_for_finalize_v1 copies rolling
--      history from the deterministic source generation into the finalizing
--      generation and trims each symbol to its latest 20 sessions.
--   2. finalize copies BEFORE publish.
--   3. destructive stale-generation cleanup runs ONLY when publish inserted
--      at least one baseline row.
-- The whole finalize RPC remains a single transaction, so any failure rolls
-- back and the source history survives untouched.
...
BEGIN
  ...
  -- Deterministic source selection: the generation (other than the target)
  -- that owns the most recent rolling volume history. Ties break on the
  -- larger session_date range, then on generation_id, so the choice is stable.
  ...
  IF array_length(v_symbols, 1) IS NOT NULL THEN
    -- Keep exactly the latest 20 sessions per symbol; the current session is
    -- never present in this table, so avg_volume_20d stays the mean of the
    -- prior 20 full regular sessions.
...
BEGIN
  ...
  -- Carry the rolling volume history forward BEFORE publishing baselines so
  -- the finalizing generation owns its own copy and cleanup can never destroy
  -- the only usable history.
  ...
  -- Fail-closed handoff: destructive volume cleanup runs ONLY after this
  -- generation actually owns published volume baselines. A zero-row publish
  -- leaves every other generation's history intact for the next attempt.
