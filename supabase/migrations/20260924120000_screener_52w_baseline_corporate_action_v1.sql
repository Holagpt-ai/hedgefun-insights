-- Forward-only: reconcile incremental 52-week extrema when split-adjusted
-- grouped-daily bars restate lower after reverse splits. Running MAX/MIN
-- without reconciliation can retain a pre-restatement high (e.g. CTNT).
...
CREATE OR REPLACE FUNCTION public.restate_screener_legacy_high(
  p_legacy_high numeric,
  p_session_high numeric
)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
...
AS $fn$
  SELECT CASE
    ...
    ELSE COALESCE(
      p_legacy_high / public.screener_near_integer_scale_factor(
        p_legacy_high / p_session_high
      ),
      p_legacy_high
    )
  END
...
CREATE OR REPLACE FUNCTION public.restate_screener_legacy_low(
  p_legacy_low numeric,
  p_session_low numeric
)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
...
AS $fn$
  SELECT CASE
    ...
    ELSE COALESCE(
      p_legacy_low * public.screener_near_integer_scale_factor(
        p_session_low / p_legacy_low
      ),
      p_legacy_low
    )
  END
...
CREATE OR REPLACE FUNCTION public.merge_screener_52w_high(
  p_existing_high numeric,
  p_bar_high numeric
)
RETURNS numeric
LANGUAGE sql
...
AS $fn$
  SELECT GREATEST(
    public.restate_screener_legacy_high(p_existing_high, p_bar_high),
    p_bar_high
  )
$fn$;

CREATE OR REPLACE FUNCTION public.merge_screener_52w_low(
  p_existing_low numeric,
  p_bar_low numeric
)
RETURNS numeric
LANGUAGE sql
...
AS $fn$
  SELECT LEAST(
    public.restate_screener_legacy_low(p_existing_low, p_bar_low),
    p_bar_low
  )
$fn$;
...
BEGIN
  ...
  ON CONFLICT (generation_id, symbol) DO UPDATE SET
    high_date = CASE
      WHEN public.merge_screener_52w_high(
        public.screener_52w_baseline_staging.high_52w,
        EXCLUDED.high_52w
      ) = EXCLUDED.high_52w
    ...
    END,
    high_52w = public.merge_screener_52w_high(
      public.screener_52w_baseline_staging.high_52w,
      EXCLUDED.high_52w
    ),
    low_date = CASE
      WHEN public.merge_screener_52w_low(
        public.screener_52w_baseline_staging.low_52w,
        EXCLUDED.low_52w
      ) = EXCLUDED.low_52w
    ...
    END,
    low_52w = public.merge_screener_52w_low(
      public.screener_52w_baseline_staging.low_52w,
      EXCLUDED.low_52w
    ),
