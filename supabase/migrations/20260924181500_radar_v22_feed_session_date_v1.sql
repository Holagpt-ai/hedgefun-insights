-- V2 candidate replace must advance surveillance session_date with the trading date.

ALTER TABLE public.radar_v22_feed_state
  ADD COLUMN IF NOT EXISTS market_data_provider text NULL,
  ADD COLUMN IF NOT EXISTS market_data_feed_mode text NULL,
  ADD COLUMN IF NOT EXISTS feed_latency_ms integer NULL,
  ADD COLUMN IF NOT EXISTS feed_connection_state text NULL,
  ADD COLUMN IF NOT EXISTS feed_last_message_at timestamptz NULL,
  ADD COLUMN IF NOT EXISTS feed_telemetry_stale boolean NOT NULL DEFAULT false;

-- Extend replace_radar_v22_candidates_v1 (optional telemetry args default NULL).

DROP FUNCTION IF EXISTS public.replace_radar_v22_candidates_v1(
