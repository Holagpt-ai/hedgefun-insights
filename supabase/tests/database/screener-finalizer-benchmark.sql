-- Isolated 52-week finalize runtime benchmark.
...
BEGIN
  ...
  FROM generate_series(0, 11949) AS g;
  ...
  FROM generate_series(0, 799) AS g;
END;
$gen$;
...
BEGIN
  ...
  END IF;
  IF v_state.symbol_count IS DISTINCT FROM 11950 THEN
  ...
  END IF;
  IF v_state.policy_excluded_count IS DISTINCT FROM 800 THEN
...
BEGIN
  ...
  IF v_inserted IS DISTINCT FROM 11950 THEN
...
BEGIN
  ...
  IF v_returned IS DISTINCT FROM 11950 THEN
...
SELECT jsonb_pretty(jsonb_build_object(
  ...
  'verdict', CASE
    WHEN max_ms <= 30000 THEN 'PASS'
    WHEN max_ms <= 45000 THEN 'CAUTION'
    ELSE 'FAIL'
  END
))
FROM stats;
